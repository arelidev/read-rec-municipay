<?php
/*
 * Plugin Name: WooCommerce NPS payment method
 * Description: Woocommerce payment method add on for NPS payments
 * Version: 1.0.2
 * Author: Wisetr
 */
...
if ( ! class_exists( 'WC_Extend_Plugin_Name' ) ) {
	...
	final class WC_Extend_Plugin_Name {
		...
		private function define_constants() {
			...
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME', $this->name );
			}
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_FILE' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME_FILE', __FILE__ );
			}
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_VERSION' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME_VERSION', $this->version );
			}
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_WP_VERSION_REQUIRE' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME_WP_VERSION_REQUIRE', $this->wp_version_min );
			}
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_WOO_VERSION_REQUIRE' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME_WOO_VERSION_REQUIRE', $this->woo_version_min );
			}
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_PAGE' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME_PAGE', str_replace( '_', '-', $this->plugin_slug ) );
			}

			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_TEMPLATE_PATH' ) ) {
				define( 'WC_EXTEND_PLUGIN_NAME_TEMPLATE_PATH', $this->template_path() );
			}

			// TODO: change 'woocommerce-extension-plugin-name' with the plugin slug of your plugin on "WordPress.org"
			if ( ! defined( 'WC_EXTEND_PLUGIN_NAME_README_FILE' ) ) {
		...
		private function check_requirements() {
			...
			if ( empty( $woo_version_installed ) ) {
				$woo_version_installed = WOOCOMMERCE_VERSION;
			}
		...
		private function register_scripts_and_styles() {
			...
			if ( is_admin() ) {
				...
				$this->load_file( $this->plugin_slug . '_admin_script', '/assets/js/admin/woocommerce-extension-plugin-name' . WC_EXTEND_PLUGIN_NAME_SCRIPT_MODE . '.js', true, array(
					'jquery',
					'jquery-blockui',
					'jquery-ui-sortable',
					'jquery-ui-widget',
